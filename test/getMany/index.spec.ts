...
describe("getMany", () => {
  it("correct response", async () => {
    const response = await dataProvider(postgrestClient).getMany!({
      resource: "posts",
      ids: ["2", "3"],
    });
  ...
  it("correct response with select metadata", async () => {
    const { data } = await dataProvider(postgrestClient).getMany!({
      resource: "posts",
      ids: ["3", "4"],
      meta: {
        select: "title",
  ...
  it("should change schema", async () => {
    ...
    const promise = dataProvider(postgrestClient).getMany({
      resource: "posts",
      ids: [1, 2],
      meta: {
        schema: "private",
