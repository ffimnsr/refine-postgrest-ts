...
describe("getList", () => {
  it("correct response", async () => {
    const { data, total } = await dataProvider(postgrestClient).getList({
      resource: "posts",
    });
  ...
  describe("PostgREST order", () => {
    const mockPostgrestOrder = jest.fn();
    const mockPostgrestClient = {
      select: () => mockPostgrestClient,
      from: () => mockPostgrestClient,
      range: () => mockPostgrestClient,
      schema: () => mockPostgrestClient,
      order: mockPostgrestOrder,
    } as unknown as PostgrestClient;

    it("correct sorting object with foreignTable", async () => {
      await dataProvider(mockPostgrestClient).getList({
        resource: "posts",
        sorters: [
          {
      ...
      expect(mockPostgrestOrder).toHaveBeenCalledWith("title", {
        ascending: true,
...
describe("filtering", () => {
  ...
  it("should change schema", async () => {
    const { data } = await dataProvider(postgrestClient).getList({
      resource: "posts",
      meta: {
        schema: "public",
        select: "*",
    ...
    const promise = dataProvider(postgrestClient).getList({
      resource: "products",
      meta: {
        schema: "private",
      },
